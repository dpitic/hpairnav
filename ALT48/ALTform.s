...
*
INCLUDE Header.H
*INCLUDE informbox48.h
*INCLUDE GUI.H
INCLUDE ALT48.h
*DEFINE FHEIGHT EIGHT    ( Redefine standard field height for HP50 )
DEFINE FHEIGHT NINE     ( Standard field height for HP48 )
*EXTERNAL DoMsgBox
...
*	 'DROPFALSE
		' ::                              ( Field Message Handler )
		    TWENTYFIVE #=case ::  ( Get internval value )
		      DUPTYPEREAL? case ::        ( REAL )
...
*	 'DROPFALSE
		' ::                                    ( Field message handler )
		    TWENTYFIVE #=case ::        ( Get internal value )
		      DUPTYPEREAL? case ::              ( REAL )
		        "ft" umEND THREE EXTN           ( Add default feet units )
